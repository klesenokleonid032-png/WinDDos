    ...
    while True:
        ...
        try:
            ...
            if zezim_classiko == "1":
                ...
                if rst == st:
                    print("Вы выиграли! Добавлен 1 бал к балансу.")
                    bal += 1
                    ...
                    print("Проигрыш...")
                    if bal == 0:
                       print("От баланса невозможно отнять бал.")
                       time.sleep(2)
                    else:
                       bal -= 1
                       print("Отнят 1 бал.")
...
def btc_sel_buy():
    ...
    while True:
        ...
        live_price = price_btc 
            
        print("--=== БАНК КРИПТОВАЛЮТ ===--")
        print(f"Ваш баланс: {bal}")
        print(f"На вашем крипто-кошельке: {btc}")
        print(f"Стоимость ДДос-коина (ОБНОВЛЯЕТСЯ РАЗ В 4 секунды): {live_price}")
        print("-------------------------")
        print("Управление: \n[1] - Купить \n[2] - Продать \n[q] - Выйти \n[Enter] - обновить цены")
        kbank = input("user>>")
        ...
        if kbank == "1":
            ...
            try:
                ...
                if btc >= sbtc:
                    ek = input("Вы уверены? [1] - да, [2] - нет \n:")
                    if ek == "1":
                        pbtc = sbtc * pfix_price
                        btc -= sbtc
                        bal += pbtc
...
def main_wdd():
    ...
    while True:
        ...
        print("Обновление новостей происходит раз в 5 секунд.")
        print("-------------------------------------")
